...
class TermsTableViewCell: UITableViewCell {
    ...
    func setupUI() {
        ...
        titleLabel.text = "本規約は、グッスミン運営局（以下、当社）が運営するサービス『グッスミン』（以下、本サービス）を通じて、本サービスを利用する利用者様（以下、利用者）に提供するサービスに関して、その諸条件を定めるものです。本サービスを利用されたことにより、本利用規約に同意いただいたものとみなします。"
        terms1Label1.text = "利用者は、本規約の定めに従って本サービスを利用しなければなりません。"
        terms1Label2.text = "本サービスは会員登録をしなくても利用することができますが、SNS機能は会員登録をしないかぎり利用（閲覧は可能）することができません。"
        terms1Label3.text = "利用者は会員登録をするにあたり、本規約に同意していただく事が必要であり、会員登録が完了した時点で、本規約を内容とする本サービス利用規約（以下、本契約）が当社との間で締結されます。"
        
        terms2Label.text = "本サービスは、サウンドの効果により安眠を促すことを目的としていますが、効果は個人差があり効果を保証するものではありません。"
